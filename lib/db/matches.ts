...
/**
 * For match detail page: fetch suggestion data for participants + top N people in DB (alphabetically)
 * This ensures that when users select someone from the full-DB search, their position/club are available for auto-fill
 */
export async function getLatestPlayerPositionByPersonIdsWithTopPeople(
  participantPersonIds: string[],
  options?: { excludeMatchId?: string; topPeopleLimit?: number }
): Promise<Record<string, PlayerPosition | null>> {
  const supabase = createServiceRoleClient()
  // Only fetch top people if participant count is low (e.g., < 30)
  // If there are many participants already, the suggestion data is likely sufficient
  if (participantPersonIds.length > 30) {
    // Just use participants
    return getLatestPlayerPositionByPersonIds(participantPersonIds, options)
  }

  const limit = options?.topPeopleLimit ?? 150

  // Fetch top N people from DB (alphabetically)
  const { data: topPeople, error: topPeopleError } = await supabase
    .from('tbl_People')
    .select('id')
  ...
  if (topPeopleError) {
    console.warn('Failed to fetch topPeople:', topPeopleError.message)
  }

  // Combine participant IDs with top people IDs
  const allPersonIds = [
    ...participantPersonIds,
    ...(topPeople?.map((p) => p.id as string) ?? []),
  ]

  // Remove duplicates
  const uniquePersonIds = [...new Set(allPersonIds)]

  // Fetch suggestion data for all
  return getLatestPlayerPositionByPersonIds(uniquePersonIds, options)
}
 ...
 * This ensures that when users select someone from the full-DB search, their position/club are available for auto-fill
 */
export async function getLatestPlayerClubTeamByPersonIdsWithTopPeople(
  participantPersonIds: string[],
  options?: { excludeMatchId?: string; targetMatchDate?: string; topPeopleLimit?: number }
): Promise<Record<string, string | null>> {
  const supabase = createServiceRoleClient()
  // Only fetch top people if participant count is low (e.g., < 30)
  // If there are many participants already, the suggestion data is likely sufficient
  if (participantPersonIds.length > 30) {
    // Just use participants
    return getLatestPlayerClubTeamByPersonIds(participantPersonIds, options)
  }
  ...
  // Fetch suggestion data for all
  return getLatestPlayerClubTeamByPersonIds(uniquePersonIds, options)
}
