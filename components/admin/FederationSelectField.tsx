...
import type { InlineCreateState, SelectOptionWithShortName } from '@/lib/types/admin'

type Option = SelectOptionWithShortName

type CreateFederationInlineAction = (
  prevState: InlineCreateState,
  formData: FormData
) => Promise<InlineCreateState>

type FederationSelectFieldProps = {
  name: string
  label: string
  selectedId?: string | null
  options: Option[]
  createFederationInlineAction: CreateFederationInlineAction
  placeholder?: string
}

export default function FederationSelectField({
  name,
  label,
  selectedId,
  options,
  createFederationInlineAction,
  placeholder = 'Wpisz, aby filtrowac federacje',
}: FederationSelectFieldProps) {
  ...
  function handleInlineCreate() {
    ...
    startTransition(async () => {
      const result = await createFederationInlineAction({ ok: false, version: 0 }, formData)
      if (result.ok && result.id && result.label) {
        const newId = result.id
        const newLabel = result.label
        setAllOptions((prev) => {
          if (prev.some((opt) => opt.id === newId)) return prev
          const next = [...prev, { id: newId, short_name: newLabel }]
          next.sort((a, b) => a.short_name.localeCompare(b.short_name, 'pl'))
          return next
        })
        setValue(newId)
        setQuery(newLabel)
  ...
  const filteredOptions = useMemo(() => {
    ...
    const matches = allOptions.filter((opt) => opt.short_name.toLowerCase().includes(q))
    matches.sort((a, b) => {
      const aStarts = a.short_name.toLowerCase().startsWith(q)
      const bStarts = b.short_name.toLowerCase().startsWith(q)
      if (aStarts && !bStarts) return -1
      if (!aStarts && bStarts) return 1
      return a.short_name.localeCompare(b.short_name, 'pl')
    })
    return matches
  }, [allOptions, query])
    ...
    <div className="flex flex-col gap-2">
      ...
      <div ref={wrapperRef} onBlur={handleWrapperBlur} className="flex flex-col gap-2">
        ...
        {isOpen && query.trim() !== '' && filteredOptions.length === 0 && (
          <p className="text-xs text-neutral-500 px-1">Brak wyników &mdash; możesz dodać nową federację poniżej.</p>
        )}
      </div>

      <div className="flex items-center justify-between gap-2">
        <p className="text-xs text-neutral-500">Wybrane: {allOptions.find((o) => o.id === value)?.short_name ?? '—'}</p>
        <button
          type="button"
      ...
      {dialogOpen && (
        <div className="rounded-lg border border-neutral-700 bg-neutral-900 p-4">
          <p className="mb-3 text-sm font-semibold text-neutral-100">Nowa federacja</p>
